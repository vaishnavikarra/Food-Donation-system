	...
	if(! ProjectDir.exists()) // if uploads directory does not exist in the application 
	{
       ...
       while (itr.hasNext())
       {
		  ...
		  */
			...
			String qry="insert into donate_food (user_id,donate_date,organization_id,food_description,total_quantity,donate_type,pic1,pic2) values ("+session.getAttribute("userid")+",curdate(),"+"'"+colvalues.get(0)+"',"+"'"+colvalues.get(1)+"',"+"'"+colvalues.get(2)+"',"+"'"+colvalues.get(3)+"',"+"'"+colvalues.get(4)+"',"+"'"+colvalues.get(5)+"')";	 
try{
...
Class.forName(JDBC_DRIVER);
	  ...
	  if(n>0)
		  out.println("<script language='javascript'>window.alert('Record Added Successfully.');window.location='user_donate_food.jsp';</script>");
	  else
		  out.println("<script language='javascript'>window.alert('Record Not Added.  Try Again!');window.location='user_donate_food.jsp';</script>");
