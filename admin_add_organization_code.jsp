	...
	if(! ProjectDir.exists()) // if uploads directory does not exist in the application 
	{
       ...
       while (itr.hasNext())
       {
		  ...
		  */
			...
			String qry="insert into donate_to (organization_name,incharge_name,mobile,email,area,address,city,state,pic1,pic2) values ("+"'"+colvalues.get(0)+"',"+"'"+colvalues.get(1)+"',"+"'"+colvalues.get(2)+"',"+"'"+colvalues.get(3)+"',"+"'"+colvalues.get(4)+"',"+"'"+colvalues.get(5)+"',"+"'"+colvalues.get(6)+"',"+"'"+colvalues.get(7)+"',"+"'"+colvalues.get(8)+"',"+"'"+colvalues.get(9)+"')";	 
try{
...
Class.forName(JDBC_DRIVER);
	  ...
	  if(n>0)
		  out.println("<script language='javascript'>window.alert('Record Added Successfully.');window.location='admin_add_organization.jsp';</script>");
	  else
		  out.println("<script language='javascript'>window.alert('Record Not Added.  Try Again!');window.location='admin_add_organization.jsp';</script>");
